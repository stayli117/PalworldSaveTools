...
3. **協力キャラクターをサーバー スロットに交換します。**
   ...
   - **ソース プレイヤー**: 協力キャラクター (`0001.sav` にホストとしてリストされているキャラクター) を選択します。
...
3. **サーバー キャラクターをホスト スロットに交換します。**
   ...
   - **ターゲット プレイヤー**: 一時的な協力キャラクター (`0001.sav` のキャラクター - ホストとしてリストされているキャラクター) を選択します。
   - ボタンをクリックしてスワップを実行します。
...
協力プレイ世界の 2 人のプレイヤーがホストを交換したい場合、たとえば、プレイヤー A がホストをしていましたが、プレイヤー B が引き継ぎたいと考えています。

**仕組み:** ホストは常に `0001.sav` を占有します。 Fix Host Save はプレーヤー A (`0001.sav`) とプレーヤー B (`XXXX.sav`) を交換し、プレーヤー B は `0001.sav` になります。次に、プレーヤー B がホストし、プレーヤー A がクライアントとして参加し、2 回目のスワップによってプレーヤー A の進行状況が新しいクライアント UID に復元されます。

**前提条件:**
- 両方のプレイヤーが以前にこのワールドに参加している必要があります (両方のプレイヤーとも `Players` フォルダーに `.sav` ファイルがあります)。
- 両方のプレイヤーが **レベル 2** 以上である必要があります。
- 開始する前に、保存フォルダー全体をバックアップしてください。
・編集中はパルワールドを終了してください。

---

**ステップ 1 — B をホスト スロットに交換します。**
- PST → **ツール** > **ホスト保存の修正** を開きます。
- 生協 `Level.sav` を参照します。
- **ソース プレーヤー**: プレーヤー A (`0001.sav`) を選択します。
- **ターゲット プレイヤー**: プレイヤー B (通常の UID) を選択します。
- スワップを実行します。現在、`0001.sav` はプレーヤー B の進行状況を保持しています。

**ステップ 2 — プレーヤー B がホストし、プレーヤー A が参加します。**
- プレイヤー B がワールドをホストします。プレイヤー A が参加し、一時的なキャラクターを作成します。 Palworld はプレーヤー A に新しい UID (例: `NEWUID.sav`) を割り当てます。
- プレイヤー A が一時キャラクターで **レベル 2** に達すると、全員がゲームを終了します。

**ステップ 3 — プレーヤー A の元の進行状況を復元します。**
- 同じ `Level.sav` を使用して **ホスト保存の修正** を再度開きます。
- **ソース プレイヤー**: プレイヤー A の元の進行状況 (現在は B の古い UID、例: `ORIGUID.sav`) を選択します。
- **ターゲット プレイヤー**: プレイヤー A の新しい一時 UID (`NEWUID.sav`) を選択します。
- スワップを実行します。プレーヤー A の元のキャラクターが新しいクライアント UID にリンクされました。

**完了。** プレーヤー B が B の元の進行状況でホストします。プレイヤー A は、A の元の進行状況で参加します。残った一時ファイルは無視するかクリーンアップできます。
