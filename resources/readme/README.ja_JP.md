...
| **クリーンアップとユーティリティ** |空のギルド、非アクティブな拠点/プレイヤー、参照されていないデータを削除します。無効なアイテム/pals/構造を削除します。ダンジョン、石油掘削装置、補給物資をリセットします。タイムスタンプを修正します。 |
...
- **Cheat Mode** — Toggle to expand all caps: level, IVs, souls, condenser rank to 255;重複を許可した無制限のアクティブ/パッシブ スキルのロックを解除します。
- **Export/Import** — Right-click any pal to export as `.pstpal` (compressed) or `.json`.パーティー、パルボックス、DPS、またはベースワーカーの空のスロットにインポートします。セーブとプレイヤー間で機能します。
- **すべての Pals** を最大化 — パーティー、すべての Palbox ページ、またはすべてのベース ワーカーのすべての pals のすべてのステータス (IVs、ソウル、ランク、レベル) を最大化します。チート モードの上限を考慮します。
...
- **クリーンアップ** — 無効/変更された項目、無効な pals および passives、無効な構造を削除します。不正な pals を修正 (合法的な最大値に制限)。対空砲塔をリセットします。 unlock private chests;すべての構造物を修復します。
...
**仕組み:** 協力プレイセーブでは、ホスト プレーヤーに `0001.sav` を使用します。専用サーバーはそうではありません。すべてのプレーヤーは通常の UID を持っています。 Fix Host Save は、サーバーが認識できるように、`0001.sav` 文字を通常の UID スロットに置き換えます。
...
**仕組み:** 同じ GUID を逆に交換します。サーバー キャラクター (通常の UID) が `0001.sav` (ホスト スロット) に交換されるため、サーバーの進行状況に合わせて協力プレイをホストできます。
...
4. **通常通り協力プレイを開催します。**
   - あなたのサーバー キャラクターがホスト (`0001.sav`) になりました。すべての進捗状況、Pals、および基地は無傷です。
