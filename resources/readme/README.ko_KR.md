...
모든 플레이어가 소유한 Pal에 대한 심층 편집 인터페이스입니다. Pals은 **파티**(액티브 스쿼드)와 **Palbox**(스토리지)로 구성됩니다.
...
**작동 방식:** 동일한 GUID를 반대로 교체합니다. 귀하의 서버 캐릭터(일반 UID)는 `0001.sav`(호스트 슬롯)으로 교체되므로 서버 진행 상황에 따라 협동을 호스트할 수 있습니다.
...
협동 세계에서 두 명의 플레이어가 호스트를 바꾸고 싶을 때(예: 플레이어 A가 호스트를 하고 있지만 플레이어 B가 그 자리를 이어받고 싶어하는 경우)

**작동 방식:** 호스트는 항상 `0001.sav`을 점유합니다. 호스트 저장 수정은 플레이어 A(`0001.sav`)를 플레이어 B(`XXXX.sav`)와 교체하므로 플레이어 B는 `0001.sav`이 됩니다. 그런 다음 플레이어 B가 호스트하고 플레이어 A가 클라이언트로 참여하고 두 번째 교체를 통해 플레이어 A의 진행 상황이 새 클라이언트 UID로 복원됩니다.

**전제조건:**
- 두 플레이어 모두 이전에 이 세계에 참가한 적이 있어야 합니다(두 플레이어 모두 `Players` 폴더에 `.sav` 파일이 있음).
- 두 플레이어 모두 **레벨 2** 이상이어야 합니다.
- 시작하기 전에 전체 저장 폴더를 백업하세요.
- 편집 중에는 Palworld를 닫으세요.

---

**1단계 - B를 호스트 슬롯으로 바꿉니다.**
- PST → **도구** → **호스트 저장 수정**을 엽니다.
- 협동조합 `Level.sav`을(를) 찾아보세요.
- **소스 플레이어**: 플레이어 A(`0001.sav`)를 선택합니다.
- **대상 플레이어**: 플레이어 B(일반 UID)를 선택합니다.
- 스왑을 실행하세요. 이제 `0001.sav`은 플레이어 B의 진행 상황을 보유합니다.

**2단계 — 플레이어 B 호스트, 플레이어 A가 합류합니다.**
- 플레이어 B가 세계를 호스팅합니다. 플레이어 A가 합류하여 임시 캐릭터를 생성합니다. Palworld는 플레이어 A에게 새로운 UID(예: `NEWUID.sav`)를 할당합니다.
- 플레이어 A가 임시 캐릭터로 **레벨 2**에 도달한 후 모두가 게임을 종료합니다.

**3단계 - 플레이어 A의 원래 진행 상황을 복원합니다.**
- 동일한 `Level.sav`을 사용하여 **Fix Host Save**를 다시 엽니다.
- **소스 플레이어**: 플레이어 A의 원래 진행 상황을 선택합니다(현재 B의 이전 UID, 예: `ORIGUID.sav`).
- **대상 플레이어**: 플레이어 A의 새 임시 UID(`NEWUID.sav`)를 선택합니다.
- 스왑을 실행하세요. 이제 플레이어 A의 원래 캐릭터가 새 클라이언트 UID에 연결됩니다.

**완료.** 플레이어 B는 B의 원래 진행 상황을 호스트합니다. 플레이어 A는 A의 원래 진행 상황에 합류합니다. 남은 임시 파일은 무시하거나 정리할 수 있습니다.
...
- macOS → `dist/PalworldSaveTools.app` → `.dmg`로 패키지됨
