...
- **심층 Pal 편집** — 통계, IVs, 영혼, 기술, passives, 작업 적합성, 순위 및 외모 플래그를 완벽하게 제어할 수 있습니다.
...
| **Pal Editor** | 통계, IVs, 영혼, 순위, 기술, passives, 작업 적합성, 보스/행운 플래그를 변경합니다. 내보내기/가져오기 pals. 불법적인 pals을 감지하고 수정합니다. 무제한 편집을 위한 치트 모드. |
...
- **모두 최대치 Pals** — 파티의 모든 pals, 모든 팔박스 페이지 또는 모든 기본 작업자에 대한 모든 통계(IVs, 영혼, 순위, 레벨)를 최대화합니다. — 치트 모드 한도를 준수합니다.
...
**작동 방식:** 협동 저장은 호스트 플레이어에 `0001.sav`을 사용합니다. 전용 서버는 그렇지 않습니다. 모든 플레이어는 일반 UID를 가지고 있습니다. 호스트 저장 수정은 `0001.sav` 문자를 일반 UID 슬롯으로 바꿔 서버가 사용자를 인식하도록 합니다.
...
**작동 방식:** 동일한 GUID를 반대로 교체합니다. 귀하의 서버 캐릭터(일반 UID)는 `0001.sav`(호스트 슬롯)로 교체되므로 서버 진행 상황에 맞춰 협동 플레이를 호스트할 수 있습니다.
