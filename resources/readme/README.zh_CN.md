...
- **全部 Pals** 最大化 — 派对中所有 pals、所有 palbox 页面或所有基地工人的所有统计数据（IVs、灵魂、等级、等级）最大化 — 尊重作弊模式上限。
...
**工作原理：** 合作社存档使用 `0001.sav` 作为主机玩家。专用服务器则不然——每个玩家都有一个常规的 UID。修复主机保存将您的 `0001.sav` 字符交换到常规 UID 插槽中，以便服务器识别您。
...
当合作世界中的两个玩家想要交换主机时，例如，玩家 A 一直在主机，但玩家 B 想要接管。

**工作原理：** 主机始终占用 `0001.sav`。修复主机保存将玩家 A (`0001.sav`) 与玩家 B (`XXXX.sav`) 交换的问题，因此玩家 B 变为 `0001.sav`。然后玩家 B 主持，玩家 A 作为客户端加入，第二次交换将玩家 A 的进度恢复到新的客户端 UID 中。

**先决条件：**
- 两名玩家必须之前加入过这个世界（两人的 `Players` 文件夹中都有 `.sav` 文件）。
- 两名玩家必须至少达到 **2 级**。
- 开始之前备份整个保存文件夹。
- 编辑时关闭 Palworld。
...
- **源玩家**：选择玩家 A (`0001.sav`)。
- **目标玩家**：选择玩家 B（常规 UID）。
- 运行交换。现在 `0001.sav` 保存着玩家 B 的进度。

**第 2 步 — 玩家 B 主持，玩家 A 加入。**
- 玩家 B 主持世界。玩家 A 加入并创建一个临时角色。 Palworld 为玩家 A 分配一个新的 UID（例如 `NEWUID.sav`）。
- 玩家 A 的临时角色达到 **2 级**，然后所有人都关闭游戏。

**第 3 步 — 恢复玩家 A 的原始进度。**
- 使用相同的 `Level.sav` 再次打开 **修复主机保存**。
- **源玩家**：选择玩家 A 的原始进度（现在位于 B 的旧 UID 中，例如 `ORIGUID.sav`）。
- **目标玩家**：选择玩家 A 的新临时 UID (`NEWUID.sav`)。
- 运行交换。玩家 A 的原始角色现在已链接到他们的新客户端 UID。

**完成。** 玩家 B 主持并播放 B 的原始进度。玩家 A 以 A 的原始进度加入。剩余的临时文件可以被忽略或清理。
