...
- **最大化所有 Pals** — 最大化队伍中所有 pals、所有 palbox 页面或所有基地工人的所有统计数据（IVs、灵魂、等级、等级）— 尊重作弊模式上限。
...
**工作原理：** 合作模式存档使用 `0001.sav` 作为主机玩家。专用服务器则不然——每个玩家都有一个常规的 UID。修复主机保存将您的 `0001.sav` 字符交换到常规 UID 插槽中，以便服务器识别您。
...
**工作原理：** 相同的 GUID 反向交换。你的服务器角色（常规 UID）会被交换到 `0001.sav` （主机槽），这样你就可以在服务器进程中主持合作游戏。
