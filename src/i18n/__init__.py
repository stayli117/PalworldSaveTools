...
def dn(category: str, raw: str, default: str | None = None) -> str:
    """展示名统一入口（display name）。

    游戏数据（帕鲁/技能/道具/科技/任务/结构…）的显示名，英文原名既当翻译键
    后缀、又当缺键回退值。集中到此处避免各处重复写
    ``t(f"{category}.{raw}", raw)``，也降低「忘包 t()」的概率。

    仅用于展示层。返回值严禁赋回任何会序列化进存档的结构
    （raw / cid / item_name / loaded_level_json 等）——那些必须保持原始英文。

    :param category: 翻译前缀，如 pal / skill / item / tech / structure / mission
    :param raw:      游戏内原始英文名（也是键后缀与回退值）
    :param default:  可选自定义回退（缺省回退为 raw 本身）
    """
    return t(f"{category}.{raw}", raw if default is None else default)
...
__all__ = ['init_language', 't', 'dn', 'set_language', 'get_language', 'load_resources', 'desc_t', 'get_native_lang_name']
