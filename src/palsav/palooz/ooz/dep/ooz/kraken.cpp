...
bool Kraken_DecodeStep(KrakenDecoder *dec, byte *dst_start, int offset, size_t dst_bytes_left_in, const byte *src,
  ...
  if ((offset & 0x3FFFF) == 0) {
    ...
    if (!src) {
      printf("ERROR: Kraken_ParseHeader returned null (invalid header)\n");
      return false;
    }
  }
  ...
  if (!src || src > src_end) {
    printf("ERROR: QuantumHeader parse invalid or overran source buffer\n");
    return false;
  }
  ...
  if (qhdr.compressed_size > (uint32)dst_bytes_left) {
    printf("ERROR: compressed_size (%u) > dst_bytes_left (%d)\n", qhdr.compressed_size, dst_bytes_left);
    return false;
  }
  ...
  if (qhdr.compressed_size == 0) {
    if (qhdr.whole_match_distance != 0) {
      if (qhdr.whole_match_distance > (uint32)offset) {
        printf("ERROR: whole_match_distance (%u) > offset (%d)\n", qhdr.whole_match_distance, offset);
        return false;
      }
  ...
  case 12:
    ...
    break;
  default:
    printf("ERROR: Unknown decoder_type %d\n", dec->hdr.decoder_type);
    return false;
  }

  if (n != qhdr.compressed_size) {
    printf("ERROR: DecodeQuantum returned %d, expected %u\n", n, qhdr.compressed_size);
    return false;
  }
