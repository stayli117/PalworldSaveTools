...
class BaseHoverOverlay(QWidget):
    ...
    def show_for_player(self, player_data: dict, global_pos: QPoint):
        ...
        self.guild_label.show()
        self.level_label.setText(f"{(t('map.info.uid') if t else 'UID')}: {player_uid}")
        self.level_label.show()
        self.leader_label.setText(f"{(t('map.info.level') if t else 'Level')}: {player_level}")
        self.leader_label.show()
        if guild_name:
            self.members_label.setText(f"{(t('map.info.guild') if t else 'Guild')}: {guild_name}")
            self.members_label.show()
        else:
            self.members_label.hide()
        self.bases_label.setText(f"{(t('map.info.base_pals') if t else 'Pals')}: {pal_count}")
        self.bases_label.show()
        self.base_id_label.setText(f"{(t('map.info.last_seen') if t else 'Last Seen')}: {last_seen}")
        self.base_id_label.show()
        self.coords_label.setText(f"{(t('map.info.location') if t else 'Location')}: X:{int(coords[0])},Y:{int(coords[1])}")
        self.coords_label.show()
        self.uid_label.hide()
